var Look = (function () {
    ...
    this.Samples.init = function () {
        ...
        $('#filter-operator').bind('change', function () {
            switch ($(this).val()) {
                case 'match'    :
                    $('#filter-value').attr('placeholder', 'regex');
                    break;
                case '=='        :
                    $('#filter-value').attr('placeholder', '');
                    break;
                case '>'        :
                case '<'        :
                    $('#filter-value').attr('placeholder', 'number');
                    break;
            }
        });
        ...
        $('#filter-apply').bind('click', function () {
            ...
            that.Socket.emit('command', { cmd: 'filter', args: [
                { key: $('#filter-key').val(), op: $('#filter-operator').val(), val: $('#filter-value').val() }
            ] });
    ...
    this.Samples.lastSample = null;

    this.Samples.add = function (buffer) {
        if (!buffer || !buffer.length) return;

        var contextSample = that.Samples;

        buffer.forEach(function (sample) {
            sample._date = new Date(sample._ts);

            if (sample._label) {
                sample._label = decodeURIComponent(sample._label);
            }
            if (sample.URL) {
                sample.URL = decodeURIComponent(sample.URL);
            }
            ...
            if (sample.Error) {
                ...
                // requests
                if (contextSample.lastSample && sample.URL === contextSample.lastSample.URL && Math.abs(sample._end - contextSample.lastSample._end) < 1) {
                    return;
                } else {
                    $('#requests-grid').data('kendoGrid').dataSource.add(sample);
                    contextSample.lastSample = sample;
                }
            } else {
                // operations
